...
const SEED_BRACKET = {
  ...
  finals: {
    ...
    t1: "NYK", t1wins: 0,
    t2: "OKC", t2wins: 0,
    note: "Series starts June 4 · Times in ARG",
  },
  confFinals: [
...
function TeamDisplay({ abbr, wins, isWinner, size = 36 }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
      ...
      {isWinner && (
        <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 9, color: "#16a34a", fontWeight: 700 }}>WON</span>
      )}
    </div>
  );
}
  ...
  return (
    <div className="glass" style={{
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: isBig ? 24 : 16 }}>
        <TeamDisplay abbr={s.t1} isWinner={s.winner === s.t1} size={isBig ? 52 : 36} />
        ...
        <TeamDisplay abbr={s.t2} isWinner={s.winner === s.t2} size={isBig ? 52 : 36} />
      </div>
...
export default function BracketTab() {
  ...
  useEffect(() => {
    fetch("/api/bracket")
      .then(r => r.json())
      .then(data => {
        // Check if finals are done via ESPN data
        ...
        if (series.length > 0) {
          ...
          if (finalsSeries?.status?.type?.name === "STATUS_FINAL") {
            setBracket(prev => ({ ...prev, playoffsActive: false }));
          }
        }
